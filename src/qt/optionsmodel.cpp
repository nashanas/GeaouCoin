...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizegeaoucoinAmount"))
        settings.setValue("nAnonymizegeaoucoinAmount", 1000);
    nAnonymizegeaoucoinAmount = settings.value("nAnonymizegeaoucoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizegeaoucoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizegeaoucoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizegeaoucoinAmount:
            return QVariant(nAnonymizegeaoucoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizegeaoucoinAmount:
            nAnonymizegeaoucoinAmount = value.toInt();
            settings.setValue("nAnonymizegeaoucoinAmount", nAnonymizegeaoucoinAmount);
            emit AnonymizegeaoucoinAmountChanged(nAnonymizegeaoucoinAmount);
            break;
        default:
            break;
        }
