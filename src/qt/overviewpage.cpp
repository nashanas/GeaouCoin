...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizegeaoucoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizegeaoucoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizegeaoucoinAmount = strAnonymizegeaoucoinAmount.remove(strAnonymizegeaoucoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizegeaoucoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizegeaoucoinAmount*COIN) nMaxToAnonymize = nAnonymizegeaoucoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizegeaoucoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizegeaoucoinAmount));
        strAnonymizegeaoucoinAmount = strAnonymizegeaoucoinAmount.remove(strAnonymizegeaoucoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizegeaoucoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizegeaoucoinAmount == 0){
