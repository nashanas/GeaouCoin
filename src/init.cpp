...
void Shutdown()
{
    ...
    RenameThread("geaoucoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: geaoucoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: geaoucoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 43232)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 43233)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizegeaoucoinamount=<n> " + _("Keep N GeaouCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. GeaouCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. GeaouCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("GeaouCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "GeaouCoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of GeaouCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart GeaouCoin to complete") << "\n";
    ...
    nAnonymizegeaoucoinAmount = GetArg("-anonymizegeaoucoinamount", 0);
    if(nAnonymizegeaoucoinAmount > 999999) nAnonymizegeaoucoinAmount = 999999;
    if(nAnonymizegeaoucoinAmount < 2) nAnonymizegeaoucoinAmount = 2;
    ...
    LogPrintf("Anonymize GeaouCoin Amount %d\n", nAnonymizegeaoucoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1GEAOU+1000 == (.1GEAOU+100)*10
       10GEAOU+10000 == (1GEAOU+1000)*10
    */
