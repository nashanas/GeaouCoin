...
int nAnonymizegeaoucoinAmount = 1000;
...
#else
    const char* pszModule = "geaoucoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GeaouCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GeaouCoin
    // Mac: ~/Library/Application Support/GeaouCoin
    // Unix: ~/.geaoucoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GeaouCoin";
#else
    fs::path pathRet;
...
void createConf()
{
	...
	const char* nodes =  "\nrpcport=43233"
						 ...
						 "\nserver=1"
						 "\nlistenonion=0"
					     "\naddnode=node1.geaoucoin.org"
						 "\naddnode=node2.geaoucoin.org"
						 "\naddnode=node3.geaoucoin.org"
						 "\naddnode=node4.geaoucoin.org"
						 "\naddnode=node5.geaoucoin.org"
						 "\naddnode=node6.geaoucoin.org"
						 "\naddnode=node7.geaoucoin.org"
						 "\naddnode=node8.geaoucoin.org"
						 "\naddnode=node9.geaoucoin.org"
						 "\naddnode=45.77.174.96"
						 "\naddnode=node10.geaoucoin.org";
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "geaoucoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "geaoucoind.pid"));
